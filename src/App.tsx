...
export default function App() {
  ...
  /* --- Odvozené (Hlavní část) — disciplíny dle oblasti --- */
  ...
  /* --- Odvozené — zaměření dle disciplíny --- */
  ...
  const isMainFilled = (() => {
    ...
    // 2) Oblast = "jiná oblast" – používá se výhradně text (areaOther, discOther, focusOther)
    if (area === "other") {
      return !!areaOther.trim() && !!discOther.trim() && !!focusOther.trim();
    }

    // 3) Standardní oblast, ale bez disciplíny
    if (!discipline) return false;

    // 4) Disciplína = "other" – povinné textové doplnění disciplíny i zaměření
    if (discipline === "other") {
      return !!discOther.trim() && !!focusOther.trim();
    }

    // 5) Standardní disciplína – musí být zaměření
    if (!focus) return false;

    // 6) Zaměření = "other" – povinné textové doplnění
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="sticky top-0 z-10 bg-white border-b">
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <span className="text-xs text-gray-500">prototyp • {date}</span>
        </div>
      </header>
          ...
          <div>
            ...
            <div className="mt-3">
              ...
              >
                ...
                <option value="jine">Neřízené zahřátí / jiné</option>
              </select>
            </div>
          </div>
          ...
          <div className="pt-4 border-t">
            ...
            {((area === "other" && discOther.trim() && focusOther.trim()) ||
              ...
              <>
                ...
                >
                  <option value="">Vyberte</option>
                  <option value="rozvoj">Rozvoj dovednosti</option>
                  <option value="upevneni">Upevnění / nácvik</option>
                  <option value="aplikace">Aplikace ve hře</option>
                  <option value="soutez">Hra / soutěžní forma</option>
                </select>
              </>
            )}
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  // Primárně použijeme sendBeacon (funguje i přes CORS), fallback je fetch s no-cors
                  if (typeof navigator !== "undefined" && "sendBeacon" in navigator) {
                    const blob = new Blob([json], { type: "application/json" });
                    (navigator as any).sendBeacon(APPS_SCRIPT_URL, blob);
                  } else {
                  ...
                  alert("❌ Nepodařilo se odeslat data: " + (error?.message || String(error)));
                }
              }}
            >
